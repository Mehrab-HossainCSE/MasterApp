...
public class SSOUserClientUpdate(IDbConnectionFactory _context, IEncryption encryption, UserCreate userCreate, ISorolSoftUserCreate
    ...
    UpdateMasterAppRole updateMasterAppRole)
{
        ...
        try
        {
                    ...
                    else if (projectId == 1)
                    {
                        ...
                        else
                        {
                            return new ProjectUserCreationResult
                            {
                                ...
                                ProjectName = "CloudPos",
                                Success = false,
                                Message = menuResult.Messages.FirstOrDefault() ?? "Menu Update  failed "
                            };
                        }
            ...
            if (!projectIds.Contains(25))
            {
                ...
                tasks.Add(Task.Run(async () =>
                {
                    ...
                    var localResult = await updateMasterAppRole.HandleAsync(dto);
